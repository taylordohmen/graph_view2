...
export default class SigmaGraphPlugin extends Plugin {
	async onload(): Promise<void> {
		...
		this.addCommand({
			...
			checkCallback: (checking: boolean): true | undefined => {
				// Conditions to check
				const sigmaView: SigmaGraphView | null = this.app.workspace.getActiveViewOfType(SigmaGraphView);
				if (sigmaView) {
					// If checking is true, we're simply "checking" if the command can be run.
					// If checking is false, then we want to actually perform the operation.
					if (!checking) {
						const gexfString: string = sigmaView.gexfString();
						const rootPath: string = this.app.vault.getRoot().path;
	...
	private async activateGraphView(): Promise<void> {
		...
		const leaves: Array<WorkspaceLeaf> = this.app.workspace.getLeavesOfType(VIEW_TYPE_SIGMA);
